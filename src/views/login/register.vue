<template>
  <div class="register">
    <div class="totle">
      <img src="./zsylogo-login.png" alt="" width="150px" height="150px">
      <span style="position: relative;bottom: 50px;font-weight: bold;font-family: '微软雅黑'">中油北斗互动营销平台</span>
    </div>
      ...
      <div class="divs" :style="{display:disone}"style="float: left;clear: both;">
        <el-form class="forms" label-position="left" label-width="120px">
          ...
          <el-form-item label="登录账户:" prop="oil_coupon_price" class="foems">
            <el-input type="test" style="width:300px;border-radius: 5px;color:black!important;" placeholder="请输入登录账户" @blur="username()" v-model="merchant_name.merchant_user_name">
            </el-input>
        ...
        <div class="divs" style="float: left;clear: both;">
          <el-form class="forms" label-position="left" label-width="130px" style="">
            <el-form-item label="企业注册信息:" prop="oil_coupon_price" class="foems">
              <el-input type="test" style="width:300px;border-radius: 5px;color:black!important;" size="small" v-model="qiyeparams.company_name"
            ...
            <span
              style="float:right;font-size: 0.6rem;color:#cccc;margin-left: 5px;text-align: left;width:160px;">仅支持jpg,文件大小
                <=2M</span> </el-form-item> <el-form-item label="合同编号:" prop="oil_coupon_price" class="foems">
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        serverUrl:'http://gwtest.youxinbao.com.cn/upload/',
    ...
    methods: {
      ...
      //商户名判断
      shname(){
        CheckMerchantName(this.mername).then(response => {
          console.log(response);
    ...
    CheckSocialCreditCode(Code)
      ...
      nextinfotwo() {
        ...
        if(this.merchant_name==""||this.merchant_user_name==""||this.user_pwd==""||this.user_pwds==""){
          this.$message({
        ...
        if(this.titleone=="visible"||this.titletwo=="visible"||this.titlethree=="visible"||this.titlefour=="visible"||!this.passwords){
          this.$message({
            message: '输入有误！请重新输入',
            type: 'error'
          })
        ...
        RegisterMerchant(this.sendusername).then(response => {
          ...
          this.actionones="http://gwtest.youxinbao.com.cn/Public/SaveImage?merchant_id="+response.data.result1+"&id=1"
          this.actiontwos="http://gwtest.youxinbao.com.cn/Public/SaveImage?merchant_id="+response.data.result1+"&id=2"
          this.actionthrees="http://gwtest.youxinbao.com.cn/Public/SaveImage?merchant_id="+response.data.result1+"&id=3"
      ...
      posturl(){
        return 'http://gwtest.youxinbao.com.cn/upload/'
      },
      previnfotwo(){
        this.disone = "block";
        this.distwo = "none";
        ...
        if(this.SYyz.contacts!=''&&this.SYyz.phone!=''&&this.phone_yz.sms_validate_code!=''&&this.contractnames&&this.contractphones){
          CheckSmsValidateCode(this.phone_yz).then(response=>{
            ...
            if(parseInt(response.data.result)===0){
              UpdateUserInfo(this.SYyz).then(response => {
                console.log(response);
              });
              this.$notify({
                title: '',
      ...
      nextinfofour(){
        ...
        if(this.qiyeparams.company_htbh!=''&&this.qiyeparams.company_tyshxydm&&this.qiyeparams.company_address&&this.qiyeparams.company_name&&this.xinyong){
          if(this.successone&&this.successtwo&&this.successthree){
            UpdateMerchant(this.qiyeparams).then(response => {
              console.log(response)
            });
            this.disthree = "none";
            this.disfour="block";
