<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-if="roleId!=6" v-model="names" clearable placeholder="全部(已开票/未开票)"
      ...
      <el-select v-if="roleId!=6" v-model="oiltype" clearable placeholder="全部(普通电子发票/增值税发票)"
    ...
    <div class="coopon-right">
      <el-button icon="el-icon-download" v-waves :loading="downloadLoading" type="primary"
                 @click="dialogokVisibles">导出</el-button>
      <el-button @click="dialogoutVisibles">确认开票</el-button>
    ...
    <el-table
      ...
      <el-table-column label="流水号" prop="oil_id" sortable="custom" align="center" width="200">
        <template slot-scope="scope">
          <span>{{ scope.row.recores_no }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="商品单价" width="150px" align="center">
        <template slot-scope="scope">
          <el-tag>{{ scope.row.unit_price }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="交易金额" width="150px" align="center">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.author }}</span>-->
          <el-tag>{{ scope.row.exchange_amount }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="折扣" width="150px" align="center">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.author }}</span>-->
          <el-tag>{{ scope.row.exchange_percent }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="消费升数" width="150px" align="center">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.author }}</span>-->
      ...
      <!--<el-table-column label="消费地点" width="150px" align="center">-->
        ...
        <!--</template>-->
      <!--</el-table-column>-->
      <el-table-column label="购方单位" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.costom_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="购方税号" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.costom_tax }}</span>
        </template>
      </el-table-column>
      <el-table-column label="开票状态" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.open_status }}</span>
        </template>
      </el-table-column>
      <el-table-column label="开票类型" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.ticket_type }}</span>
        </template>
      </el-table-column>
      <el-table-column label="消费地点" width="150px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="地址电话" width="200px" align="center">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.oil_change_date | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
          <span>{{ scope.row.phone }}</span>
        </template>
      </el-table-column>
      <el-table-column label="开户行及账号" width="150px" align="center">
        <template slot-scope="scope">
          <span></span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="邮箱号码" width="200px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="申请时间" width="200px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.apple_time|parseDateTime }}</span>
        </template>
      </el-table-column>
      <el-table-column label="开票时间" width="200px" align="center" >
        <template slot-scope="scope">
          <span v-if="scope.row.open_time!=null">{{ scope.row.open_time|parseDateTime }}</span>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!--导出-->
    <el-dialog :visible.sync="kaipiao" title="提示">
      <div>您选择了<el-tag>已开票</el-tag>的数据，继续导出请点击<el-tag>确定</el-tag></div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="kaipiao = false">取 消</el-button>
        <el-button type="primary" @click="dialogkaipiao">{{ $t('table.confirm') }}</el-button>
      </span>
    </el-dialog>
    ...
    methods: {
      ...
      SelectionChange:function(val){
        ...
        for(var i=0;i<this.checkedinvoices.length;i++){
          if(this.list_query.coupon_list==''||this.list_query.coupon_list==null){
            ...
            this.checkedprice+=this.checkedinvoices[i].exchange_amount
        }
        this.checkedprice=this.checkedprice.toFixed(2)
         // alert("aa")
        if(this.checkedinvoices.length==0){
          this.bussinessTotalMoney=this.sum_price;
          this.total=this.totals
        }
        else{

          this.bussinessTotalMoney=this.checkedprice;
          this.total=this.checkedinvoices.length
        }
      ...
      dialogoutVisibles(){
        ...
        if(this.checkedlen<=0){
          ...
          for(var i=0;i<this.checkedinvoices.length;i++) {
            if (this.checkedinvoices[i].open_status == "已开票") {
              len++
            }
          }
          if(len==0){
      ...
      clickdescType(item){
        ...
        if(this.ascdesctxt==1&&this.inx==1){
          this.list=this.list.reverse(function(a,b){
            return Date.parse(b.apple_time) - Date.parse(a.apple_time);
          });
          this.inx=0
        }
      ...
      compare(obj1,obj2){
        var val1 = parseInt(obj1.apple_time);
        var val2 = parseInt(obj2.apple_time);
        if(val1 < val2){
          return 1;
        }else if(val1 > val2){
          return -1;
      ...
      getList() {
        ...
        GetCouponTicketRecods(this.listQuery).then(response => {
          ...
          if (response.data.data.length > 0) {
            ...
            this.sum_price=response.data.sum_amount
            this.total = response.data.total_count
            this.totals=response.data.total_count
      ...
      handleFilterDate(item) {
        ...
        // console.log(this.listQuery.begin_date);
        // console.log(this.listQuery.end_date);
        this.getList()
      },
      ...
      handleFilterExchangeType(name) {
        this.listQuery.pageNum = 1
        this.listQuery.open_status =name
      ...
      handleDownload() {
        this.downloadLoading = true
        this.checkedinvoices.forEach(obj => {
          if(obj.apple_time){
            obj.apple_time=new Date(parseInt(obj.apple_time.replace("/Date(", "").replace(")/", ""), 10));
            obj.apple_time=this.getDateTime(obj.apple_time);
            console.log(obj.apple_time)
          }
        })
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['流水号', '商品名称', '商品单价', '交易金额', '折扣', '消费升数', '消费地点', '购方单位', '购方税号', '开票状态',
            '开票类型','消费地点','地址电话','开户行及账号','备注','邮箱时间','申请时间','开票时间']
          const filterVal = ['recores_no', 'consumable', 'oil_l', 'unit_price', 'exchange_percent', 'exchange_amount', 'station_name', 'station_name', 'costom_name','open_status','ticket_type','station_name','',
            'costom_tax','','email','apple_time','open_time']
          const data = this.formatJson(filterVal, this.checkedinvoices)
