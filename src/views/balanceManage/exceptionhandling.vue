<template>
  <div class="app-container" v-loading="listLoadings">
    <div class="filter-container">
      ...
      <el-select v-model="names" clearable placeholder="商户号"
      ...
      <el-button @click="getList" type="info" style="margin-left: 20px;">查看处理信息</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="异常商户"  align="center" width="200">
        <template slot-scope="scope">
          <span>{{ scope.row.merchant_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="电子券码" width="180px" align="center">
        <template slot-scope="scope">
      ...
      >
        <template slot-scope="scope">
          ...
          <i v-if="scope.row.status === 1" class="fa fa-circle text-danger ml5" style="color: green"/>
        </template>
      </el-table-column>
      <!--<el-table-column label="操作" width="150pGetMerchantCouponExceptionx" align="center">-->
        <!--<template slot-scope="scope">-->
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.listQuery.begin_date='2019-01-01'
    ...
    methods: {
      ...
      getlistall(callback){
        ...
        // for(var i=0;i<this.listlen;i++){
          this.listQueryall.pageNum++
          // console.log(JSON.stringify(this.listQueryall))
          var parames=this.listQueryall
          GetMerchantCouponException({
      ...
      getList() {
        var _this=this
        this.listLoadings = true
        if(this.listQuery.merchant_id==''){
          this.listQuery.merchant_id=0
        }
        GetMerchantCouponException(this.listQuery).then(response => {
          console.log(response);
          if (response.data.data.length > 0) {
            ...
            this.listlen=parseInt(this.total/10)
            var len=this.total%10
            if(len>0){
              this.listlen++
            }
            //this.getlistall()
      ...
      handleDownload() {
        ...
        this.getlistall(function(){
          ...
          _this.listall.forEach(item => {
            var obj=item
            obj.log_time= eval('new ' + (obj.log_time.replace(/\//g, '')));
            var month=obj.log_time.getMonth() + 1
            var day=obj.log_time.getDate()
            var hours=obj.log_time.getHours()
            var minutes=obj.log_time.getMinutes()
            var seconds=obj.log_time.getSeconds()
            if(month<10){
              month="0"+month
            }
            if(hours<10){
            ...
            item.log_time= obj.log_time.getFullYear() + '-' + month + '-' + day + ' '
          ...
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['异常商户', '电子券码', '记录创建时间', '状态']
            const filterVal = ['merchant_name', 'coupon_no', 'log_time', 'status']
