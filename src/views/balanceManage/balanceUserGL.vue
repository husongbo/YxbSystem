<template>
  <div class="app-container">
    ...
    <el-table
      ...
      @sort-change="">
      <el-table-column label="企业注册信息" width="250px" align="center">
      ...
      <el-table-column label="开票密钥" width="250px">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.oil_change_date | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
          <el-tag type="info">{{ scope.row.merchant_coupon_private_key }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="商户名称" width="120px">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.oil_change_date | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
          <span type="info">{{ scope.row.merchant_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="电子券密钥" width="250px">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.oil_change_date | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
          <el-tag type="info">{{ scope.row.merchant_private_key }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="核销回调地址" width="250px">
        <template slot-scope="scope">
          <!--<span>{{ scope.row.oil_change_date | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
          <el-tag type="info">{{ scope.row.merchant_url }}</el-tag>
        </template>
      </el-table-column>
    ...
    <pagination
      :total="total"
      :page.sync="listQuery.pageNum" :pageSizes="[50,100,150,200]"
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible">
      <el-form
        ...
        <el-form-item label="核销回调地址" prop="oil_base_price">
          <el-input v-model="edituserinfos.merchant_url" clearable/>
        </el-form-item>
        <el-form-item label="电子券密钥" prop="oil_change_date">
          <el-input v-model="edituserinfos.merchant_private_key" disabled="disabled" style="font-size: 16px;" clearable/>
        </el-form-item>
        <el-form-item label="开票密钥" prop="oil_change_date">
          <el-input v-model="edituserinfos.merchant_coupon_private_key" clearable/>
        </el-form-item>
        </el-form-item>
...
<script>
  ...
  import { getBalanceInfoList, getBalanceHistoryInfoList, subMitBalanceApplyInfo,GetRegisterMerchantList,GetMerchantTechnologyList,UpdateMerchantTechnology} from '@/api/balanceManage'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        GetMerchantTechnologyList(this.listQuery).then(response => {
      ...
      createData() {
        UpdateMerchantTechnology(this.edituserinfos).then((res)=>{
          console.log(res)
      ...
      handleUpdate(row) {
        ...
        this.edituserinfos.merchant_id=this.edituserinfo.id
        this.edituserinfos.merchant_url=this.edituserinfo.merchant_url
        this.edituserinfos.merchant_private_key=this.edituserinfo.merchant_private_key
        this.edituserinfos.merchant_coupon_private_key=this.edituserinfo.merchant_coupon_private_key
